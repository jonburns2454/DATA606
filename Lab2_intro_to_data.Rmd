...
The point of using a histogram is to discover and evaluate the distribution of a data set. Since bins determine the width of the intervals, you can control how much data is present in x number of bins. In the histogram with size 15 bins, you can much clearly see that a large portion of the data is more frequent in the lower dep_delays. However, there are too many bins to determine the full shape of the distribution. On the other had the 150 binwidth gives us only three bars, making it equally as hard to determine distribution. The benefit of the larger binwidth plot is you can better see an outlier falling around 375 dep_delay. The first plot with no specified binwidth seems to show the best distribution, showing a high concentration around the lowest times, and then a clear petering off as the times get higher and less frequent.
